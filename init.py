...
def download_file(url: str, download_path: Path, target_name: str, use_request: bool = False, use_cache: bool = False, sha256: Optional[str] = None, verbose: bool = False, proxy: Optional[str] = None) -> bool:
    ...
    if use_cache and target_file.exists() and (get_sha256(target_file) == sha256 if sha256 else True):
        ...
        if use_request:
            ...
            subprocess.check_call([CURL, '-L', url, '-o', temp_download_path] + ([] if verbose else ['--silent']) + (['--proxy', proxy] if proxy else []), env=curl_environ, stdout=subprocess.DEVNULL)
        if sha256:
...
def do_enable_pip(proxy: Optional[str], local_file: Optional[Path] = None, use_request: bool = False, verbose: bool = False):
    ...
    if local_file is not None:
        ...
        success = download_file('https://bootstrap.pypa.io/get-pip.py', DownloadsDir, 'get-pip.py', use_request=use_request, use_cache=True, verbose=verbose, proxy=proxy)
        if success:
...
def main(args):
    ...
    do_enable_pip(proxy=args.proxy, local_file=ProgramHome / 'scripts' / 'get-pip-3.8.py', verbose=args.verbose)
    ...
    for tool, tool_info in tools.tool_itmes():
        ...
        success = download_file(url, DownloadsDir, tool_download_name, use_request=args.use_request, use_cache=True, sha256=sha256, verbose=args.verbose, proxy=args.proxy)
        if not success:
            continue
